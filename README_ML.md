...
4. **Суммаризация текста** - конденсация длинного текста в краткое содержание (русский и английский).
...
- **`ParaphraserProcessor`** — процессор для перефразирования текста.
...
- **'SummarizerProcessor'** - процессор для суммаризации текста
...
3. **scikit-learn** — библиотека для ML  
   - Пока используется только для векторизации текста.
...
- Русский — модель IlyaGusev/rut5_base_sum_gazeta.

- Английский — модель facebook/bart‑large‑cnn.

- Поддержка длинных текстов: автоматическая разбивка на куски (chunk_size, по умолчанию 512 токенов) с последующим объединением частичных резюме.

- Параметры запроса: language, max_length, min_length, do_sample, chunk_size.
